...
function planet_ui_init()
    addEventHandler("onClientRender", resourceRoot, planet_ui_show)
end
...
function planet_ui_prepare()
    ...
    local planet_galaxy = planet_getPlanetGalaxy(player_planet) -- ?
    ...
    if string.size(planet_name) > GUI_MAX_PLANET_NAME_CHARS then
        planet_name = string.sub(planet_name, 1, GUI_MAX_PLANET_NAME_CHARS - 2)
        planet_name = planet_name .. ".."
    end
    if string.size(planet_galaxy) > GUI_MAX_PLANET_NAME_CHARS then
